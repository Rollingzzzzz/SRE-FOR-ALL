#!/usr/bin/env python3
"""LLM kalıp-keşif çağrısı — python_scripts/call_llm_for_pattern_recon.py (K1-K8 uyumlu).

Görev (high-level):
  chunk.py çıktısındaki dilimi (_SOL_/_EOL_ formatında) alır, config klasöründeki
  prompt dosyalarıyla LLM'e sorgu atar, dönen cevabın metnini dosyaya yazar.
  Kalıp çıkarımı (pattern recon): LLM'den tekrarlayan satır kalıplarını etiket ve
  regex kuralları olarak JSON halinde ister — bunlar sonraki aşamada log
  elemede kullanılır.

Prompt dosyaları DIŞARIDA durur (kod değişmeden düzenlenebilir):
  <config-dir>/system_prompt.txt  — sistem mesajı
  <config-dir>/user_prompt.txt    — kullanıcı mesajı şablonu; içindeki
                                    $hadoop placeholder'ı girdi içeriğiyle
                                    değiştirilir (--placeholder ile değişir)
  Varsayılan config klasörü: bu script'in yanında <script_adı>_config/

K6 (wire log): her deneme ./llm/<script_adı>_NNNN_request.json ve
  _NNNN_response.json olarak yazılır; numara mevcut en büyükten devam eder
  (restart'a dayanıklı), hata durumunda bile response yazılır. API anahtarı
  wire loglara ASLA yazılmaz (yalnızca maskeli parmak izi).

API anahtarı çözüm sırası: --api-key > --api-key-file (KEY=VALUE dosyası,
  varsayılan /secrets/zai.env) > ZAI_API_KEY ortam değişkeni.
  Model ve base-url de aynı dosyadan/ortamdan (ZAI_MODEL, ZAI_BASE_URL) düşer.
...
log = logging.getLogger("call_llm")
...
def http_post_json(url: str, body: dict, api_key: str, timeout: float) -> tuple:
    """POST atar (stream yok); (http_status, parsed_json_or_None, raw_text, elapsed) döner.
...
def http_post_json_stream(url: str, body: dict, api_key: str, idle_timeout: float,
    ...
    idle_timeout = veri TAMAMEN susarsa iptal süresi. Delta'lar aktıkça
    (düşünme/cevap arttıkça) koşu istediği kadar sürebilir — kesilmez.
    İlerleme görülmüşken akış düşerse LLMStreamInterrupted fırlatır (retry yok,
    kısmi içerik korunur); hiç veri gelmeden düşerse LLMNetworkError (retry OK)."""
    body = dict(body)
    body["stream"] = True
    data = json.dumps(body, ensure_ascii=False).encode("utf-8")
    req = urllib.request.Request(
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        prog="call_llm_for_pattern_recon.py",
        description="chunk.py dilimini LLM'e gönderip kalıp (pattern) kuralları ister. "
                    "Promptlar <config-dir> altındaki dosyalardan okunur (kod değişmeden "
                    "düzenlenebilir). Her çağrı/yanıt --llm-dir altına JSON yazılır (K6).")
